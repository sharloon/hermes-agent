...
class TestSendToPlatformDiscordMedia:
    ...
    def test_media_files_passed_on_last_chunk_only(self):
        ...
        with patch("tools.send_message_tool._send_discord", side_effect=mock_send_discord):
            result = asyncio.run(
                _send_to_platform(
                    ...
                    "999",
                    long_msg,
                    media_files=[("/fake/左侧菜单栏样式.png", False)],
                )
            )
        ...
        assert call_log[1]["media_files"] == [("/fake/左侧菜单栏样式.png", False)]  # Last chunk: media attached
    ...
    def test_single_chunk_gets_media(self):
        ...
        assert call_kwargs["media_files"] == [("/fake/左侧菜单栏样式.png", False)]
